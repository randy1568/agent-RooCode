...
export class Task extends EventEmitter<TaskEvents> implements TaskLike {
	...
	private async initiateTaskLoop(userContent: Anthropic.Messages.ContentBlockParam[]): Promise<void> {
		...
		while (!this.abort) {
			...
			// as he can.
			/**  这个智能体循环的工作方式是，会给克莱因分配一个任务，然后他会调用工具来完成该任务。除非有 attempt_completion 调用，否则我们会一直用他的工具的响应来回复他，直到他要么调用 attempt_completion，要么不再使用任何工具。如果他不再使用任何工具，我们会让他考虑自己是否已经完成了任务，然后调用 attempt_completion，否则就继续完成任务。存在一个 MAX_REQUESTS_PER_TASK 限制，以防止无限请求，但会提示克莱因尽可能高效地完成任务。*/
	...
	public async recursivelyMakeClineRequests(
		...
		while (stack.length > 0) {
			...
			/** 获取详细信息是一项耗时的操作，它会使用 ripgrep 来
			// 自顶向下构建项目的文件结构，对于大型项目而言，这可能
			// 需要几秒钟时间。为了获得最佳的用户体验，在此过程中，我们会显示一个带有加载 spinner 的占位符 api_req_started 消息。 */

			// Determine API protocol based on provider and model
			const modelId = getModelId(this.apiConfiguration)
			const apiProtocol = getApiProtocol(this.apiConfiguration.apiProvider, modelId)

			// 显示加载状态：发送 api_req_started 消息
			await this.say(
				"api_req_started",
				JSON.stringify({
					request:
					...
					while (!item.done) {
						...
						switch (chunk.type) {
							...
							case "text": {
								...
								// Present content to user.包含文本和tool call信息
	...
	public async *attemptApiRequest(retryAttempt: number = 0): ApiStream {
		...
		//上下文截断处理
		if (contextTokens) {//检查是否有上下文token
			/**
			 * 从API处理器获取当前模型的详细信息
